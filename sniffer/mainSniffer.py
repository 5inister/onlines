...
# Quick analysis: sniff(iface='wlan0',prn=quickAnalysis,store=0)
...
services={'facebook':'facebook','fbcdn':'facebook','fbstatic':'facebook','twitter':'twitter','twvid':'twitter','twimg':'twitter','flickr':'flickr','instagram':'instagram','whatsapp':'whatsapp','softlayer':'whatsapp','skype':'skype','youtube.com':'youtube','googlevideo':'youtube','ytimg':'youtube'}
...
def getRouterMac():
        '''Gets the router's mac address as a Long number,
        conerts it to hex and formats it to a familiar 
        represantation.
        Takes:
        Nothing
        Returns:
	...
	macaddr=''
        mac=get_mac()
        mac=hex(mac)
        for i in range(2,len(mac)-1,2):
                macaddr+=mac[i]+mac[i+1]+':'
        macaddr=macaddr[0:-1]
        return macaddr 
...
def submitReport(server,report): #TODO: Publish to php
	...
	Nothing
	'''
        #FOR THE TIME BEING WRITES TO FILE
        with open(server,'a') as outFile:
                outFile.write(report+',')
                
def quickAnalysis(packet):
        '''
